//
//  Animal.h
//  TakeMeHome
//
//  Created by Nigel on 2015/8/18.
//  Copyright (c) 2015年 Josh. All rights reserved.
//

#import <Foundation/Foundation.h>


@interface Animal : NSObject

@property (nonatomic , strong) NSString *animal_area_pkid;
@property (nonatomic , strong) NSString *animal_kind;
@property (nonatomic , strong) NSString *animal_age;
@property (nonatomic , strong) NSString *animal_sex;
@property (nonatomic , strong) NSString *resourceStr;
@property (nonatomic , strong) NSString *animal_bodytype;
@property (nonatomic , strong) NSString *album_file;
@property (nonatomic , strong) NSString *animal_id;
@property (nonatomic , strong) NSString *animal_place;
@property (nonatomic , strong) NSString *animal_favorite;
@property (nonatomic , strong) NSString *animal_rsc;
@property (nonatomic , strong) NSString *animal_user_post_icon;


@end
